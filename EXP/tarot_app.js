document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawCard(card) {
        ...
        let orientFocus = "";
        if (isReversed) {
            orientFocus = "牌面為【逆位】：請探討能量的阻塞、內在的抗拒、延遲、過度執著或需要反思的盲點，並給出如何「疏通與釋放」的建議。";
        } else {
            orientFocus = "牌面為【正位】：請探討能量的順暢流動、外在的顯化優勢，並給出如何「把握與順勢而為」的建議。";
        }
...
# 你的任務 (Task)
請以溫暖、客觀且具洞察力的語氣，為詢問者進行解讀。請嚴禁教科書式的背誦，請像一位睿智的朋友般對話，並包含以下三個結構：

1. 【能量共振】：${topicFocus}，解釋這張牌 (${card.name} ${orient}) 在此情境下，傳遞了什麼核心訊息？
2. 【破局建議】：針對上述狀態，給出 1~2 個「具體、可落地」的行動方案或心態轉換建議。
3. 【靈魂拷問】：最後，請根據這張牌的深層意涵，提出 1 個直擊內心的反思問題，引導詢問者自我對話。
